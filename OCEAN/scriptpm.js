...
new TimelineMax({onStart:populate})
		...
		.set('.foundTxt',	{left:0, top:30, width:1024, fontSize:35, textAlign:'center', textContent:'Encuentra el par perfecto...', fontWeight:300, letterSpacing:0.25})
...
function populate() {
    ...
    matched = false;
		msTilHint = 5000;
		TweenMax.set(hintBtn, {autoAlpha:0, textContent:"Ayuda?", fontSize:25});
    ...
    if (matched) return; //prevent user from repeatedly clicking the matching pair
        ...
        matched = true;
				...
				new TimelineMax({onComplete:populate})
					.set('.foundTxt', {textContent:'Puntos: '+found, fontWeight:500}, 0)
...
function updateTime(){
		if (timeLeft>0){
			...
			TweenMax.set('.endTxt', {textContent:hint+" Otro Intento? "+hint, fontSize:35});
